...
float getAngle(float time) {
  return sin(time * speed) * 1.2 + time * -0.1;
}
...
void main () {
  ...
  ang += box.y * (ang - getAngle(time - 0.01)) * 20.;
  box = rotate(box, ang);
  box += 0.5;

  float s = 0.5 - (cos(time * speed) * 0.2 * size + size) * 0.5;
